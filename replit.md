...
- Schema changes applied via `pnpm --filter @workspace/db run push` (drizzle-kit push, not migration files)
...
- **`dashboard.ts`** — 8 count queries consolidated into 1 SQL with scalar
  subqueries; full-row visit fetch replaced with `SUM(duration_minutes)`.
  Total: 5 SQL statements (counts + weekDelivered + weekSchedules +
  activeRule + caregivers). Reduced from 11.
...
`lib/db/src/index.ts` uses `AsyncLocalStorage` + a custom Drizzle logger
that increments a counter on every `logQuery` call. `app.ts` wraps each
`/api` request in the counter context and patches `res.json` to inject
`X-Query-Count` with the real SQL statement count. The fk-index-audit
script independently verifies indexes exist via `pg_indexes`.
...
| `anomaly.scan-visit`, `anomaly.scan-all`, `auth.predict-renewal`, `auth.predict-renewals-all`, `compliance.daily-scan`, `pay-period.auto-close` | 1 | Serialized to avoid resource contention |

### Pusher channel lifecycle

Pusher is used in `artifacts/caregiver-pwa/src/pages/Messages.tsx` for
real-time message delivery on per-thread private channels
(`private-thread-<threadId>`). The `useEffect` has three `cancelled`
checkpoints (after credentials fetch, after dynamic import, after client
construction). Cleanup calls `ch.unbind()` → `client.unsubscribe()` →
`client.disconnect()` in that order. Scale: 50 caregivers × 1 active
thread = 50 Pusher connections (free tier supports 200). Falls back to
15s polling when Pusher credentials are unavailable. No PUSHER_KEY/SECRET
is configured in this environment, so runtime Pusher integration testing
requires adding those secrets first.
